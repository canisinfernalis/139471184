...
local Library = {
    ...
    ToggleInteractionCooldown = 1,
    CollapsibleStartCollapsed = true,
    NotifyOnError = false,
    DebugEnabled = false,
    DebugScopes = {},
    AsyncErrorCooldown = 2,
    AsyncLastErrorTick = {},
    Throttles = {},
    SignalGroups = {},
    ...
    Scheme = {
        ...
        ChromeColor = Color3.fromRGB(8, 8, 8),
...
local function GetSchemeValue(Index)
    ...
    if ReplaceAliasIndex and Library.Scheme[ReplaceAliasIndex] ~= nil then
        return Library.Scheme[ReplaceAliasIndex]
    end
...
local function WaitForEvent(Event, Timeout, Condition)
    ...
    while not Fired and (os.clock() - Start) < (Timeout or 5) do
        task.wait()
    end
...
local function DeferredResize(Target, ...)
    local Args = { n = select("#", ...), ... }
    task.defer(function()
        if Target and type(Target.Resize) == "function" then
            if type(UnpackFn) == "function" then
                Target:Resize(UnpackFn(Args, 1, Args.n))
            else
                Target:Resize()
            end
        end
...
local function FilterElementList(Elements, ParentMatch, Search)
    ...
    for _, ElementInfo in Elements do
        ...
        elseif ElementInfo.SubButton then
            local Visible = false
            local TextMatch = ParentMatch or (ElementInfo.Text and ElementInfo.Text:lower():find(Search, 1, true))
            local SubTextMatch = ParentMatch or (ElementInfo.SubButton.Text and ElementInfo.SubButton.Text:lower():find(Search, 1, true))

            if TextMatch and ElementInfo.Visible then
                Visible = true
                ElementInfo.Base.Visible = true
            else
                ElementInfo.Base.Visible = false
            end

            if SubTextMatch and ElementInfo.SubButton.Visible then
                Visible = true
                ElementInfo.SubButton.Base.Visible = true
            else
        ...
        local Visible = (ParentMatch or (ElementInfo.Text and ElementInfo.Text:lower():find(Search, 1, true))) and ElementInfo.Visible
        ElementInfo.Holder.Visible = Visible == true
        if Visible then
            VisibleElements += 1
        end
    end
...
local function CheckDepbox(Box, Search, OverrideMatch)
    ...
    VisibleElements += FilterElementList(Box.Elements, Match, Search)
...
local function ApplySearchToTab(Tab, Search)
    ...
    for _, Groupbox in Tab.Groupboxes do
        ...
        VisibleElements += FilterElementList(Groupbox.Elements, GroupboxMatch, Search)
    ...
    for _, Tabbox in Tab.Tabboxes do
        ...
        for _, SubTab in Tabbox.Tabs do
            ...
            VisibleElements[SubTab] += FilterElementList(SubTab.Elements, SubTabMatch, Search)
...
function Library:DisconnectSignalGroup(GroupName)
    if type(GroupName) ~= "string" or GroupName == "" then
        return
    end

    local group = Library.SignalGroups[GroupName]
    if type(group) ~= "table" then
        Library.SignalGroups[GroupName] = nil
        return
    end

    for index = #group, 1, -1 do
        local connection = table.remove(group, index)
        if typeof(connection) == "RBXScriptConnection" and connection.Connected then
            pcall(function()
                connection:Disconnect()
            end)
        end
        Library:RemoveArrayItem(Library.Signals, connection)
    end
...
local function UnpackArgs(packed)
    ...
    return UnpackFn(packed, 1, packed.n)
end

function Library:ConnectSignalGroup(GroupName, Signal, Callback)
    if not (type(GroupName) == "string" and GroupName ~= "" and Signal and type(Callback) == "function") then
        return nil
    end

    if type(Library.SignalGroups[GroupName]) ~= "table" then
        Library.SignalGroups[GroupName] = {}
    end

    local connection = Signal:Connect(function(...)
        local packed = PackArgs(...)
        local ok, err = xpcall(function()
            Callback(UnpackArgs(packed))
        end, debug.traceback)
        if not ok then
            Library:WarnThrottled(
                "SignalGroupError:" .. tostring(GroupName),
                2,
                "[Library] Signal group error [" .. tostring(GroupName) .. "]: " .. tostring(err)
            )
        end
    end)

    table.insert(Library.SignalGroups[GroupName], connection)
...
local function NormalizeAsyncInvocation(contextOrCallback, callbackOrArg, ...)
    if type(contextOrCallback) == "function" then
        return "async", contextOrCallback, PackArgs(callbackOrArg, ...)
    end
    if type(contextOrCallback) == "string" and contextOrCallback ~= "" and type(callbackOrArg) == "function" then
        return contextOrCallback, callbackOrArg, PackArgs(...)
    end
    if type(contextOrCallback) == "table" and type(contextOrCallback.Name) == "string" and type(callbackOrArg) == "function" then
        return contextOrCallback.Name, callbackOrArg, PackArgs(...)
    end
    return nil, nil, nil
end

function Library:PackArgs(...)
    return PackArgs(...)
end

function Library:NormalizeAsyncInvocation(contextOrCallback, callbackOrArg, ...)
    return NormalizeAsyncInvocation(contextOrCallback, callbackOrArg, ...)
end

function Library:GetToggleValue(container, key, defaultValue)
    local entry = type(container) == "table" and container[key] or nil
...
function Library:GetOptionValue(container, key, defaultValue)
    local entry = type(container) == "table" and container[key] or nil
    local value = type(entry) == "table" and entry.Value or nil
    if value ~= nil then
        return value
...
function Library:GetOptionTable(container, key)
    local value = Library:GetOptionValue(container, key, nil)
...
function Library:GetOptionString(container, key, defaultValue)
    local fallback = defaultValue or ""
    local value = Library:GetOptionValue(container, key, fallback)
...
function Library:GetOptionNumber(container, key, defaultValue)
    local value = tonumber(Library:GetOptionValue(container, key, defaultValue))
    if value ~= nil then
        return value
    end
...
function Library:ThrottleReady(Key, Cooldown)
    if type(Key) ~= "string" or Key == "" then
        return true
    end

    local now = os.clock()
    local cooldown = tonumber(Cooldown) or 0
    return (now - (Library.Throttles[Key] or 0)) >= cooldown
end

function Library:UseThrottle(Key)
    if type(Key) ~= "string" or Key == "" then
        return
    end

    Library.Throttles[Key] = os.clock()
end

function Library:WarnThrottled(Key, Cooldown, ...)
    if not Library:ThrottleReady(Key, Cooldown) then
        return
    end

    Library:UseThrottle(Key)
    warn(...)
end

function Library:WarnAsyncThrottled(Scope, Err)
    local key = tostring(Scope or "async")
    local now = os.clock()
    if (now - (Library.AsyncLastErrorTick[key] or 0)) < Library.AsyncErrorCooldown then
        return
    end

    Library.AsyncLastErrorTick[key] = now
    warn("[Library] Async callback error [" .. key .. "]: " .. tostring(Err))
end

function Library:RunProtectedAsync(Callback, PackedArgs, Scope)
    if type(Callback) ~= "function" then
        return false, "invalid callback"
    end

    local ok, err = xpcall(function()
        Callback(UnpackArgs(PackedArgs or PackArgs()))
    end, debug.traceback)

    if not ok then
        Library:WarnAsyncThrottled(Scope, err)
    end

    return ok, err
end

function Library:TaskSpawn(contextOrCallback, callbackOrArg, ...)
    local scope, callback, packed = NormalizeAsyncInvocation(contextOrCallback, callbackOrArg, ...)
    if type(callback) ~= "function" then
        return nil
    end

    return task.spawn(function()
        Library:RunProtectedAsync(callback, packed, scope or "task.spawn")
    end)
end

function Library:TaskDefer(contextOrCallback, callbackOrArg, ...)
    local scope, callback, packed = NormalizeAsyncInvocation(contextOrCallback, callbackOrArg, ...)
    if type(callback) ~= "function" then
        return nil
    end
    ...
    return task.defer(function()
        Library:RunProtectedAsync(callback, packed, scope or "task.defer")
    end)
end

function Library:TryCall(Func, ...)
...
function Library:TryWarn(Label, Func, ...)
    local results = PackArgs(Library:TryCall(Func, ...))
    if results[1] ~= true then
        warn("[Library] " .. tostring(Label or "Operation") .. " error: " .. tostring(results[2]))
    end
    return UnpackArgs(results)
end

function Library:WaitUntil(Label, Timeout, Interval, Predicate)
    if type(Predicate) ~= "function" then
        return false
    end

    local deadline = os.clock() + (tonumber(Timeout) or 0)
    local step = tonumber(Interval) or 0.1
    local lastErr = nil
    repeat
        local ok, result = pcall(Predicate)
        if ok and result == true then
            return true
        elseif not ok and result ~= lastErr then
            lastErr = result
            Library:DebugLog("WaitUntil", "%s predicate error: %s", tostring(Label or "condition"), tostring(result))
        end
        task.wait(step)
    until os.clock() >= deadline

    Library:DebugLog("WaitUntil", "%s timed out after %.2fs", tostring(Label or "condition"), tonumber(Timeout) or 0)
    return false
end

function Library:RunBootstrapSection(Label, Func, Fatal)
    local ok, err = xpcall(Func, debug.traceback)
    if ok then
        return true
    end

    local prefix = "[Midgard:" .. tostring(Label or "Bootstrap") .. "] "
    if Fatal == true then
...
function Library:DisableIdledConnections()
    pcall(function()
        ...
        for _, connection in getConnectionsFn(LocalPlayer.Idled) do
...
function Library:StartPauseBypass(Config)
    ...
    if getconnections then
        pcall(function()
            for _, connection in getconnections(LocalPlayer:GetPropertyChangedSignal("GameplayPaused")) do
                if connection.Disable then
                    connection:Disable()
                elseif connection.Disconnect then
                    connection:Disconnect()
...
local function BuildAssetIcon(iconName: string, assetId: number): Icon
    return {
        Url = string.format("rbxassetid://%d", assetId),
        Id = assetId,
        IconName = iconName,
...
function Library:Validate(Table: { [string]: any }, Template: { [string]: any })
...
local function FillInstance(Table: { [string]: any }, Instance: GuiObject)
    ...
    if next(ThemeProperties) ~= nil then
...
function Library:GetTextBounds(Text: string, Font: Font, Size: number, Width: number?): (number, number)
    ...
    TextBoundsParams.Width = Width or workspace.CurrentCamera.ViewportSize.X - 32
...
function Library:AddListener(listeners, func)
    ...
    for _, existing in listeners do
...
function Library:TrackConnection(array, connection, connectionSet)
    if type(array) ~= "table" or typeof(connection) ~= "RBXScriptConnection" then
        return connection
    end

    if type(connectionSet) == "table" then
        if connectionSet[connection] then
            return connection
        end
        connectionSet[connection] = true
    else
        for _, existing in array do
    ...
    table.insert(array, connection)
    return connection
end

function Library:UntrackConnection(array, connection, connectionSet)
    if type(array) ~= "table" or typeof(connection) ~= "RBXScriptConnection" then
        return
    end

    if type(connectionSet) == "table" then
        connectionSet[connection] = nil
    end

    for index = #array, 1, -1 do
        if array[index] == connection then
...
function Library:PruneConnectionArray(array, targetConnection, connectionSet, disconnectRemoved)
    if type(array) ~= "table" then
        return
    end

    for index = #array, 1, -1 do
        local existing = array[index]
        local shouldRemove = existing == targetConnection
            or typeof(existing) ~= "RBXScriptConnection"
            or existing.Connected ~= true

        if shouldRemove then
            if type(connectionSet) == "table" then
                connectionSet[existing] = nil
            end
            if disconnectRemoved == true and typeof(existing) == "RBXScriptConnection" and existing ~= targetConnection and existing.Connected then
                existing:Disconnect()
            end
            array[index] = array[#array]
            array[#array] = nil
...
function Library:DisconnectConnectionArray(array, connectionSet)
    if type(array) ~= "table" then
        return
    end

    for index = #array, 1, -1 do
        local connection = array[index]
        if typeof(connection) == "RBXScriptConnection" and connection.Connected then
            connection:Disconnect()
        end
        array[index] = nil
...
function Library:RemoveTableReference(container, key, value)
    ...
    for entryKey, entryValue in container do
...
function Library:DestroyComponent(component)
    ...
    if type(component.Addons) == "table" then
        for _, addon in component.Addons do
...
function Library:SafeCallback(Func: (...any) -> ...any, ...: any)
    ...
    local Result = PackArgs(xpcall(Func, function(Error)
        local trace = debug.traceback(Error, 2)
        Library:WarnAsyncThrottled("SafeCallback", trace)
    ...
    return UnpackFn(Result, 2, Result.n)
end
...
function Library:GetDiagnostics()
    ...
    return {
        ...
        WindowCount = GetTableSize(Library.Windows),
        TabCount = GetTableSize(Library.Tabs),
        NotificationCount = GetTableSize(Library.Notifications),
        DialogCount = GetTableSize(Library.Dialogues),
        ContextMenuCount = GetTableSize(Library.ContextMenus),
        SignalCount = GetTableSize(Library.Signals),
...
function Library:ClearRegistries()
    for _, registry in Library.Registries or {} do
...
function Library:AddBoundControl(Tab, MethodName, Idx, Info, OnChanged)
    if type(Tab) ~= "table" or type(MethodName) ~= "string" or type(Tab[MethodName]) ~= "function" then
        return nil
    end

    Tab[MethodName](Tab, Idx, Info)

    local control = Options[Idx] or Toggles[Idx]
    if control and type(control.OnChanged) == "function" and type(OnChanged) == "function" then
        control:OnChanged(OnChanged)
    end

    return control
end

function Library:AddLabeledToggle(Tab, Idx, Text, DefaultValue, OnChanged)
    return Library:AddBoundControl(Tab, "AddToggle", Idx, {
        Text = Text,
...
function Library:BindExclusiveDropdownGroup(Dropdowns, FallbackValues)
    ...
    local fallbackList = {}
    for _, value in type(FallbackValues) == "table" and FallbackValues or {} do
    ...
    local function normalize(forceDefault)
        ...
        local allowed = {}
        local allowedList = {}
        for _, dropdown in dropdownList do
        ...
        for index, dropdown in dropdownList do
            local value = if forceDefault then nil else dropdown.Value
...
function Library:MakeDraggable(UI: GuiObject, DragFrame: GuiObject, IgnoreToggled: boolean?, IsMainWindow: boolean?)
    ...
    local Changed
    Library:GiveSignal(DragFrame.InputBegan:Connect(function(Input: InputObject)
...
function Library:AddDraggableButton(Text: string, Func, ExcludeScaling: boolean?)
    ...
    function Table:SetIcon(IconName: string, Size: number?)
        ...
        Button.Text = ""
        Button.Size = UDim2.fromOffset(iconSize, iconSize)
...
function Library:Unload()
    ...
    for GroupName in Library.SignalGroups do
        Library:DisconnectSignalGroup(GroupName)
    end
    ...
    -- Clear globals
    local env = getgenv()
    local activeSaveManager = env.MidgardSaveManager
    if type(activeSaveManager) ~= "table" or activeSaveManager.Library == Library then
        env.MidgardSaveManager = nil
    end
    if env.Midgard_Running then env.Midgard_Running = false end
    ...
    if env.MidgardLibrary == Library then
        env.MidgardLibrary = nil
    end
end

local CheckIcon = BuildAssetIcon("check", 136734716346836)
local ArrowIcon = BuildAssetIcon("chevron-up", 97701193495928)
local ResizeIcon = BuildAssetIcon("move-diagonal-2", 121257222570504)
local KeyIcon = BuildAssetIcon("key", 113969619760808)
local MoveIcon = BuildAssetIcon("move", 81927347916588)
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        local AreModifiersHeld = function(Required)
            if not (typeof(Required) == "table" and next(Required) ~= nil) then
                return true
            end
        ...
        function KeyPicker:SetValue(Data)
            ...
            KeyPicker.DisplayValue = if next(KeyPicker.Modifiers) ~= nil
...
do
    ...
    function Funcs:Toggle(Idx, Text, Default, Callback)
        return self:AddToggle(Idx, {
            Text = Text,
    ...
    function Funcs:Slider(Idx, Text, Min, Max, Default, Rounding, Callback)
        return self:AddSlider(Idx, {
            Text = Text,
            Min = Min or 0,
            Max = Max or 100,
    ...
    function Funcs:Dropdown(Idx, Text, Values, Default, Multi, Callback)
        return self:AddDropdown(Idx, {
            Text = Text,
            Values = Values or {},
            Default = Default,
            Multi = Multi == true,
    ...
    function Funcs:Input(Idx, Text, Placeholder, Default, Numeric, Callback)
        return self:AddInput(Idx, {
            Text = Text,
    ...
    function Funcs:Keybind(Idx, Text, Default, Mode, Callback)
        local Label = self:AddLabel(Text)
        return Label:AddKeyPicker(Idx, {
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        local Toggle = {
            Text = Info.Text,
            Value = Info.Default,
            InteractionCooldown = tonumber(Info.InteractionCooldown),
            LastUserInteractionTick = 0,
        ...
        function Toggle:SetInteractionCooldown(Cooldown: number?)
            Toggle.InteractionCooldown = tonumber(Cooldown)
        end
        ...
        Button.MouseButton1Click:Connect(function()
            ...
            local cooldown = Toggle.InteractionCooldown
            if cooldown == nil then
                cooldown = Library.ToggleInteractionCooldown
            end
            cooldown = math.max(tonumber(cooldown) or 0, 0)
            local now = os.clock()
            if cooldown > 0 and (now - (Toggle.LastUserInteractionTick or 0)) < cooldown then
                return
            end
            Toggle.LastUserInteractionTick = now
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        function Dropdown:GetSelectedCount()
            if Info.Multi then
                local count = 0
                for _, enabled in Dropdown.Value or {} do
                    if enabled then
                        count += 1
                    end
                end
        ...
        function Dropdown:IsSelectionValid()
            if Info.Multi then
                for value, enabled in Dropdown.Value or {} do
        ...
        function Dropdown:SetSelectionMap(SelectionMap)
            ...
            for value, enabled in SelectionMap do
                if enabled then
        ...
        function Dropdown:SetSelectionList(SelectionList)
            ...
            if Info.Multi then
                local map = {}
                for _, value in SelectionList do
                    map[value] = true
                end
        ...
        function Dropdown:BuildDropdownList()
            local Values = type(Dropdown.Values) == "table" and Dropdown.Values or {}
            ...
            for _, Value in Values do
                ...
                if not IsDisabled then
                    Button.MouseButton1Click:Connect(function()
                        ...
                        local activeCount = 0
                        if Info.Multi then
                            for _, v in Dropdown.Value do if v then activeCount += 1 end end
                        else
                        ...
                        if not (activeCount <= minSel and not Try) then
                            ...
                            for _, OtherButton in DropdownButtons do
        ...
        function Dropdown:SetValue(Value)
            ...
            Dropdown.PendingValue = if fullyApplied then nil else Value
            local changed
            if Info.Multi then
            ...
            Dropdown:Display()
            for _, Button in DropdownButtons do
                Button:UpdateButton()
            end
        ...
        function Dropdown:SetValues(Values, PreserveSelection)
            ...
            Dropdown.Values = type(Values) == "table" and Values or {}
    ...
    function Funcs:MakeCollapsible(StartCollapsed: boolean?, HideDivider: boolean?)
        ...
        local ArrowIcon = BuildAssetIcon("chevron-down", 128259194661926)
        ...
        function self:ApplyCollapse()
            ...
            local Icon = self.Collapsed
                and BuildAssetIcon("chevron-up", 97701193495928)
                or BuildAssetIcon("chevron-down", 128259194661926)
        ...
        if StartCollapsed then
            ...
            task.defer(function()
                ...
                local Icon = BuildAssetIcon("chevron-up", 97701193495928)
                Arrow.Image = Icon.Url
                Arrow.ImageRectOffset = Icon.ImageRectOffset
                Arrow.ImageRectSize = Icon.ImageRectSize
            end)
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        local SearchIcon = BuildAssetIcon("search", 76330464544459)
        if SearchIcon then
            New("ImageLabel", {
    ...
    function Window:ForEachTab(Callback)
        ...
        for key, value in Window.Tabs do
            Callback(key, value)
        end
    end

    function Window:ClearRegistries()
        for _, registry in Window.Registries do
            table.clear(registry)
        end
    end
    ...
    function Window:Destroy()
        ...
        for _, Dialog in Window.Dialogues do
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:SetVisible(Visible)
                ...
                if Tabbox.ActiveTab then
                    Tabbox.ActiveTab:Resize()
                    DeferredResize(Tabbox.ActiveTab)
                end
            end
    ...
    function Window:AddKeyTab(...)
        ...
        Icon = Icon or "rbxassetid://113969619760808"
    ...
    function Window:AddDialog(Idx, Info)
        ...
        function Dialog:Destroy()
            ...
            for buttonIdx in FooterButtonsList do
                table.insert(buttonsToRemove, buttonIdx)
            end
            for _, buttonIdx in buttonsToRemove do
...
function Library:BuildConfigPanel(settingsTab, saveManager, options)
    ...
    local boxIcon = panelOptions.Icon or "rbxassetid://95940780435079"
    local tabName = panelOptions.TabName or ""
    local tabIcon = panelOptions.TabIcon or "rbxassetid://122811892296432"
...
function Library:BuildControlSnapshot(toggleIndexes, optionIndexes, ignoreMap)
    ...
    for _, idx in toggleIndexes or {} do
        toggleAllow[idx] = true
    end
    for _, idx in optionIndexes or {} do
...
function Library:MergeControlSnapshots(...)
    ...
    for i = 1, select("#", ...) do
        ...
        if type(snapshot) == "table" then
            for idx, entry in snapshot.Toggles or {} do
                merged.Toggles[idx] = entry
            end
            for idx, entry in snapshot.Options or {} do
...
function Library:ReplayControlSnapshot(snapshot, replayOptions)
    ...
    local function resolveDropdownSingle(option, entry)
        ...
        for _, candidate in option.Values or {} do
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            for _ = 1, retryCount do
                for idx, entry in pendingToggles do
                ...
                for idx, entry in pendingOptions do
        ...
        if not ok then
            warn("[Library:LoadConfig] " .. tostring(err))
        end
    end)
end
